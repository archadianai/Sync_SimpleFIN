...
const CONN_METHOD =
	"sync_simplefin.sync_via_simplefin.doctype.simplefin_connection.simplefin_connection";
...
frappe.ui.form.on("SimpleFIN Connection", {
	refresh(frm) {
		...
		if (frm.fields_dict.account_mappings) {
					...
					grid_row.show_form = function () {
						...
						if (!$actions.data("rearranged")) {
							...
							[$move, $collapse, $cancel, $delete].forEach(function ($btn) {
								...
								$btn.attr("style",
									"padding:4px 10px; font-size:12px; line-height:18px; " +
									"height:26px; float:none; border-width:0; box-sizing:border-box;"
								);
							});
			...
			$grid.find(".grid-static-col").css({"height": "auto", "max-height": "none"});
			$grid.find(".data-row").css("height", "auto");
		}
